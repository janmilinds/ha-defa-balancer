...
**Directory:** `custom_components/defa_balancer/[platform]/`
...
```python
"""[Platform] platform for DEFA Balancer."""
...
from .entity import DEFABalancerEntity
from .[entity_file] import DEFABalancer[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: DEFABalancerDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            DEFABalancer[EntityName](coordinator, entry),
...
```python
"""[Entity description] for DEFA Balancer."""
...
Add platform to `custom_components/defa_balancer/manifest.json`:
...
- [ ] Entity class inherits from both `DEFABalancerEntity` and platform class
...
- **Class prefix:** `DEFABalancer`
- **Base entity:** `DEFABalancerEntity` in `entity/base.py`
- **Coordinator:** `DEFABalancerDataUpdateCoordinator`
