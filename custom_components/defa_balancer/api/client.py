...
class DEFABalancerApiClientError(Exception):
    """Base exception to indicate a general API error."""


class DEFABalancerApiClientCommunicationError(
    DEFABalancerApiClientError,
):
    """Exception to indicate a communication error with the API."""


class DEFABalancerApiClientAuthenticationError(
    DEFABalancerApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        DEFABalancerApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise DEFABalancerApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class DEFABalancerApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            DEFABalancerApiClientAuthenticationError: If authentication fails.
            DEFABalancerApiClientCommunicationError: If communication fails.
            DEFABalancerApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise DEFABalancerApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise DEFABalancerApiClientError(
                msg,
            ) from exception
